...
class InMemoryRedisMock {
    ...
    private geoStore = new Map<string, Map<string, { lon: number; lat: number }>>();
    ...
    async georadius(
        ...
        const results: { member: string; distance: number }[] = [];
        ...
        for (const [member, coords] of keyMap.entries()) {
            ...
            let distance = distKm;
            if (unit === "m") distance = distKm * 1000;
            else if (unit === "mi") distance = distKm * 0.621371;
            else if (unit === "ft") distance = distKm * 3280.84;
