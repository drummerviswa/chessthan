...
export const activeTournaments = new Map<string, Tournament>();
...
export const joinTournament = (tournamentId: string, userId: string | number, userName: string) => {
...
const generateSwissRound = (t: Tournament) => {
...
export const triggerArenaPairings = (t: Tournament) => {
...
export const registerMatchResult = (
    tournamentId: string,
    gameCode: string,
    winnerId: string | number | null,
    isDraw = false
    ...
    if (t.type === "arena") {
        ...
        const activeMatches = t.matches.filter(
...
const completeTournament = (t: Tournament) => {
    t.status = "completed";
...
const broadcastLobbyUpdate = (t: Tournament) => {
