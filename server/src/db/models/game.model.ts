...
export const save = async (game: Game) => {
    try {
        const white: { id?: number; name?: string } = {};
        const black: { id?: number; name?: string } = {};
        ...
        if (white.id || black.id) {
            ...
            const eloFieldMap: Record<string, string> = {
                bullet: "eloBullet",
                blitz: "eloBlitz",
                rapid: "eloRapid",
            ...
            if (white.id) {
                ...
                if (wUser) whiteElo = (wUser as any)[eloField];
            }
            if (black.id) {
                const bUser = await prisma.user.findUnique({ where: { id: black.id } });
                if (bUser) blackElo = (bUser as any)[eloField];
            }
